...
class ImageStorage {
    ...
    String getUrl(final ImageHolder holder, ImageFormat format = null) {
        fileStorage.getUrl(holder.imagesPath, getFilename(format ?: holder.defaultImageFormat), holder.imagesBucket)
    }
    ...
    void delete(final ImageHolder holder, ImageFormat format = null) {
        if (format) {
            fileStorage.delete(holder.imagesPath, getFilename(format), holder.imagesBucket)
        } else {
            for (ImageFormat f in holder.imageFormats) {
                fileStorage.delete(holder.imagesPath, getFilename(f), holder.imagesBucket)
            }
        }
    }
}
